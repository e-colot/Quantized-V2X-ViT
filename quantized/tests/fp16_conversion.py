...
quantizer = AffineFakeQuantizer({'type' : 'fp16'})
...
print("Overflows check ----------------------------------------- passed")
    ...
    # Subnormal values
# fp16 min normal is ~6.1e-5, smaller values become subnormal
subnormal_values = torch.tensor([1e-6, -1e-6], dtype=torch.float32)
...
reference_random = x_random.to(torch.float16).to(torch.float32)
match_mse = torch.nn.functional.mse_loss(quantized_random, reference_random).item()
print("Random input, comparison with built-in fp16 conversion ---------")
print("\n                MSE = {}".format(match_mse))

if match_mse <= 1e-10:
    print("\nFP16 quantizer ----------------------------------------- success")
